...
$(function() {
    ...
    $.post('../ngym/GymEmployeesAction!list.zk', {
        ...
        if (data.STATUS) {
            ...
            $('#Year').combobox('select',new Date().getFullYear())
            $('#Month').combobox('select', new Date().getMonth()+1)
            getDateAllList({'year':new Date().getFullYear(),'month':new Date().getMonth()+1})
        } else {
...
function getDateAllList(dat){
    $('#dg').datagrid({
        url: '../evaluation/EvaluationAction!getByMonth.zk',
        pageSize: '30',
        method: 'post',
        queryParams: dat,
...
function getCYM(){
    return {'coachId':$('#roleSel').combobox("getValue"),'year':$('#Year').combobox("getValue"),'month':$('#Month').combobox("getValue")};
}
function openIframe() {
    var row = $('#dg').datagrid('getSelected');
    if($('#roleSel').combobox("getValue") == ""){
        $.messager.alert('&nbsp;&nbsp;&nbsp;&nbsp;提示', '请筛选教练！');
        return;
    }
    if (!row) {
...
/*    var jiaolianId = $('#roleSel').combobox("getValue");
    if (!jiaolianId) {
        $.messager.alert('&nbsp;&nbsp;&nbsp;&nbsp;提示', '请先选择教练!');
        return;
    }*/
    $.ajax({
        type:'post',
        dataType:'json',
        url:'../evaluation/EvaluationAction!getAvgEvaluationByMonth.zk',
        data:getCYM(),
        success: function(res){
            if(typeof(res.rows.avgProfessional)=='undefined'){
                $('#pro').text('暂无分数');
                $('#server').text('暂无分数');
            }else{
                $('#pro').text(res.rows.avgProfessional);
                $('#server').text(res.rows.avgService)
            }           
            $('#iframeInfo').dialog('open').dialog('setTitle', '&nbsp;&nbsp;&nbsp;'+$('#roleSel').combobox("getText")+'教练'+$('#Year').combobox("getValue")+'年'+$('#Month').combobox("getValue")+'月'+'平均分');;
        }
    })
